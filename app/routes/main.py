...
def api_sync_event():
    ...
    # Capture user info before spawning the background thread so that
    # sync_from_config() (which relies on current_user / Flask `g`) can
    # resolve the scouting team number correctly.
    _user_id = current_user.id
    _scouting_team = getattr(current_user, 'scouting_team_number', None)

    # Grab a reference to the app that survives outside the request.
    ...
    def run_sync_background():
        ...
        sync_from_config() in both teams.py and matches.py relies on Flask
        globals that only exist inside a **request** context:
          - ``current_user``  (Flask-Login)
          - ``session`` / ``g``  (Flask)
          - ``flash()`` / ``url_for()`` / ``redirect()``

        We push a *test* request context so every downstream helper finds
        the globals it expects.  After each sync call we collect any
        ``flash()`` messages and store them in the results dict.
        """
        try:
            with _app.test_request_context('/api/sync-event', method='POST'):
                # Authenticate the user inside this fabricated request so
                # that current_user, g.scouting_team_number, etc. work.
                from flask_login import login_user as _login_user
                from flask import g
                _user = User.query.get(_user_id)
                if _user:
                    _login_user(_user, remember=False)
                # Also set g.scouting_team_number for mobile-API-style
                # helpers that check g before current_user.
                ...
                try:
                    ...
                    # Collect flash messages generated by sync_from_config
                ...
                # Ensure the session is clean before we're done
                try:
                    db.session.remove()
                except Exception:
                    pass
            ...
            _app.logger.exception(f'Background sync {sync_id} failed')
            with active_syncs_lock:
