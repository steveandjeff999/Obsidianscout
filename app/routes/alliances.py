...
def team_details():
    """Return team details for use in the recommendations overlay."""
    team_number = request.args.get('team_number')
    team_id = request.args.get('team_id', type=int)
    event_id = request.args.get('event_id', type=int)

    if not team_number and not team_id:
        return jsonify({'error': 'team_number or team_id required'}), 400

    # Find the team by number or id (id is a fallback)
    team = None
    if team_number:
        team = Team.query.filter_by(team_number=team_number).first()
    ...
    try:
        # Basic metrics (auto/teleop/endgame/total)
        analytics_result = calculate_team_metrics(team.id, event_id=event_id)
        metrics = analytics_result.get('metrics', {}) or {}

        # Build prediction elements from config (scoring elements with display_in_predictions)
        game_config = get_effective_game_config()
        prediction_elements = []

        def _add_element(elem, period_name):
            if not elem.get('display_in_predictions'):
                return
            element_id = elem.get('perm_id') or elem.get('id')
            if not element_id:
                return

            prediction_elements.append({
                'id': element_id,
                'name': elem.get('name') or element_id,
                'period': period_name,
                'type': elem.get('type'),
                'points': elem.get('points'),
                'options': elem.get('options') if isinstance(elem.get('options'), list) else None
            })

        for period_name in ['auto_period', 'teleop_period', 'endgame_period']:
            period = game_config.get(period_name, {})
            for elem in period.get('scoring_elements', []):
                _add_element(elem, period_name.replace('_period', ''))

        # Gather simple per-element averages from scouting data
        element_stats = []
        if prediction_elements:
            from app.utils.analysis import get_analysis_data_for_team
            scouting_records = get_analysis_data_for_team(team.id, event_id)

            # Build stats for each element
            for elem in prediction_elements:
                elem_id = elem['id']
                values = []
                for rec in scouting_records:
                    v = rec.data.get(elem_id)
                    if v is not None:
                        values.append(v)
                ...
                elif elem['type'] in ('boolean',):
                    # percent true
                    true_count = sum(1 for v in values if v in (True, 'true', 'True', 1, '1'))
                    avg_value = (true_count / len(values)) * 100
                else:
                    # select/multiple_choice or other - show most common
                    from collections import Counter
                    counter = Counter(values)
                    most_common, count = counter.most_common(1)[0]
                    avg_value = most_common

                # Convert to points if possible
                avg_points = None
                if elem.get('points') is not None and isinstance(elem.get('points'), (int, float)):
                    # if boolean and avg is percent, approximate points = percent * points
                    if elem['type'] == 'boolean' and isinstance(avg_value, (int, float)):
                        avg_points = avg_value / 100.0 * elem['points']
                    elif isinstance(avg_value, (int, float)):
                        avg_points = avg_value * elem['points']
                elif elem.get('options') and isinstance(avg_value, str):
                    # Map option name to points
                    for opt in elem['options']:
                        if isinstance(opt, dict) and opt.get('name') == avg_value:
                            avg_points = opt.get('points')
                            break
                ...
                element_stats.append({
                    ...
                    'type': elem['type'],
                    'avg_value': avg_value,
                    'avg_points': round(float(avg_points), 1) if isinstance(avg_points, (int, float)) else None
                })
        ...
        response = {
            ...
            'metrics': {
                'auto_points': round(float(metrics.get('auto_points', 0)), 1),
                'teleop_points': round(float(metrics.get('teleop_points', 0)), 1),
                'endgame_points': round(float(metrics.get('endgame_points', 0)), 1),
                'total_points': round(float(metrics.get('total_points', 0)), 1)
            },
            'prediction_elements': element_stats
        }

        return jsonify(response)
    except Exception as e:
        current_app.logger.exception('Error getting team details')
