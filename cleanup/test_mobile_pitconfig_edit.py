...
def test_update_and_restore_pit_config():
    """Login as admin, fetch pit_config, modify via mobile API, verify, then restore."""
    ...
    resp = requests.get(PIT_URL, headers={'Authorization': f'Bearer {token}'}, verify=False, timeout=10)
    ...
    marker_key = '__mobile_api_pit_test_marker__'
    marker_value = f"test-{int(time.time())}"
    modified[marker_key] = marker_value

    try:
        save_resp = requests.post(PIT_URL, headers=headers, json=modified, verify=False, timeout=10)
        ...
        resp2 = requests.get(PIT_URL, headers={'Authorization': f'Bearer {token}'}, verify=False, timeout=10)
        ...
        assert cfg2.get(marker_key) == marker_value, f"Marker not found in saved pit config: {cfg2.get(marker_key)}"

    finally:
        try:
            restore_resp = requests.put(PIT_URL, headers=headers, json=original_config, verify=False, timeout=10)
            ...
            print(f"Warning: failed to restore pit config: {e}")
